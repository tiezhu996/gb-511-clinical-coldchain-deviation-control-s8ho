...
type ExcursionEvent struct {
	...
	// Assessment* fields snapshot the automatic rule evaluation performed at
	// registration time so the detail view can show 触发温度、允许时长和结果.
	AssessmentOutcome string    `json:"assessmentOutcome" gorm:"size:40;index"`
	AssessmentNote    string    `json:"assessmentNote" gorm:"size:1000"`
	AllowedMinutes    int       `json:"allowedMinutes"`
